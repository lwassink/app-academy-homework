...
class MetaCorgiPerkPackage
  ...
  ShoppingList.new.methods.each do |method_name|
    match = method_name.to_s[/get_(.*)_info/, 1]
    if match
      self.define_perk(match.downcase.to_sym)
    end
  end
end
...
if __FILE__ == $PROGRAM_NAME
  ...
  3.upto(3) do |i|
    perks = MetaCorgiPerkPackage.new(list, i)
    p perks.bone
    p perks.kibble
    p perks.silly_outfit
  end
end
